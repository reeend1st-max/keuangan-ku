...
(function () {
  ...
  var Api = {
    ...
    register: async function (username, password, name) {
      ...
      if (!cleanName) throw new Error("Nama lengkap / nama akun wajib diisi.");

      var email = toEmail(cleanUser);
      var res = await sb.auth.signUp({
        email: email,
        password: password,
        options: { data: { name: cleanName, username: cleanUser } },
      });
      ...
      return {
        user: { id: res.data.user.id, username: cleanUser, name: cleanName, email: email },
      };
    },
